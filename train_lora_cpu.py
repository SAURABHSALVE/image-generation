...
def add_lora(unet, rank=4):
    lora_attn_procs = {}
    for name, module in unet.attn_processors.items():
        cross_dim = unet.config.cross_attention_dim if "cross_attention_dim" in unet.config else None
        hidden_size = unet.config.block_out_channels[0]
        lora_attn_procs[name] = LoRAAttnProcessor(
            hidden_size=hidden_size,
            cross_attention_dim=cross_dim,
...
def main():
    ...
    add_lora(unet, rank=4)
    params = lora_params(unet)
    optimizer = bnb.optim.AdamW8bit(params, lr=1e-4)
    ...
    while global_step < max_steps:
        for batch in loader:
            ...
            if global_step % 50 == 0 and global_step > 0:
                torch.save(unet.attn_processors.state_dict(), f"lora_out/lora_step_{global_step}.bin")
            global_step += 1
    ...
    if accelerator.is_main_process:
        torch.save(unet.attn_processors.state_dict(), "lora_out/lora_out_cpu.bin")
        print("Final LoRA saved to lora_out/lora_out_cpu.bin")
